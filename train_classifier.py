...
if __name__ == "__main__":
    ...
    # Filter for only first two classes (0 and 1)
    train_mask = torch.tensor(train_dataset.targets) < 2
    val_mask = torch.tensor(val_dataset.targets) < 2
    ...
    train_targets = torch.stack([torch.tensor(x[1]) for x in train_dataset]).cuda()
    train_dataset = torch.utils.data.TensorDataset(
        train_data, torch.tensor(train_targets).cuda()
    )
    val_data = torch.stack([x[0] for x in val_dataset]).cuda()
    val_targets = torch.stack([torch.tensor(x[1]) for x in val_dataset]).cuda()
    val_dataset = torch.utils.data.TensorDataset(val_data, val_targets.cuda())
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, load_batch_size, shuffle=True
    )
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, load_batch_size, shuffle=False
    )
